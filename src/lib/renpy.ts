import type { ExtractMode, DialogItem } from './types';
...
export function maskTagsInText(text: string) {
  const s = String(text ?? '');
  if (!s) return { masked: s, map: Object.create(null) as Record<string, string> };
...
let MODE: ExtractMode = 'safe';

export function setMode(mode: ExtractMode) {
  const v = String(mode || '').toLowerCase().trim() as ExtractMode;
...
function computeBlockMasks(lines: string[]) {
  ...
  const stack: Array<{ type: string; indent: number }> = [];
...
function isWrappedByUnderscore(source: string, openQuoteStart: number) {
...
function escapeForRenpyString(text: string, quoteChar: '"' | "'", isTriple: boolean) {
  let t = String(text ?? '');
  if (!isTriple) {
    t = t.replace(/\r?\n/g, '\\n');
  ...
  const delim = quoteChar + quoteChar + quoteChar;
  const esc = '\\' + delim;
  return t.split(delim).join(esc);
}

export function extractDialogs(source: string, mode: ExtractMode) {
  setMode(mode);
  ...
  const byLine = new Map<number, typeof literals>();
  for (const lit of literals) {
    if (!byLine.has(lit.startLine)) byLine.set(lit.startLine, []);
    byLine.get(lit.startLine)!.push(lit);
  }

  const dialogs: Array<Omit<DialogItem, 'id' | 'fileId' | 'idx' | 'updatedAt'>> = [];
  ...
  for (const [lineIdx, list] of byLine.entries()) {
    ...
    for (const lit of list) {
      ...
      if (NON_TRANSLATABLE_ATTRS.has(prevId) || NON_TRANSLATABLE_CALLS.has(prevId) || prevId === '_') continue;
...
export function applyTranslations(
  source: string,
  dialogs: Array<Pick<DialogItem, 'contentStart' | 'contentEnd' | 'translated' | 'quoteChar' | 'isTriple'>>,
  eol: '\n' | '\r\n',
  creditLine: string,
) {
  ...
  if (credit) {
    if (!out.includes(credit)) out = out + nl + nl + credit + nl;
    else out = out + nl;
  } else {
    out = out + nl;
  }
